...
- I've view several ways to do with BEM about modifier, I stick to the first I encounter: `.x-block--modifier`
...
About units for expressing length, I tried to avoid using `px` because it's no longer revelant in the variety of devices a content can be viewed, but for more clarity I use `rem` with a little trick at first: I set the base value of `1rem` to an equivalent of `10px`, so the math is easy to translate a design with the units expressed in `px`.
...
Because consistancy is very valuable, sticking to widely shared standards will bring constraints, but these will help keep a healthy code base. Various tools will help to apply those constraints, like [Prettier](https://prettier.io/) in combinaition with [stylelint](https://stylelint.io/).
